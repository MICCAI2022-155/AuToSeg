...
def test():
    import numpy as np
    y_true = tf.Variable(np.ones((2,2, 2)))
    y_true = y_true[0,0,0].assign(0)
    y_true = y_true[1,1,0].assign(0)

    y_true = y_true[0,1,0].assign(2)


    y_true[:,:,0]

    y_pred = tf.Variable(np.zeros((2,2, 2)))
    y_pred[0,1,1].assign(0.9)
    y_pred[0,1,0].assign(0.1)

    y_pred[0,0,0].assign(1)

    y_pred[1,0,0].assign(1)
